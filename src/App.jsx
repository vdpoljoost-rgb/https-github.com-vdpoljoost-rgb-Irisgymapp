...
// ---------- Fallback mini-lijst (als JSON faalt) ----------
...
function useExerciseDB(data){
  const [list,setList]=useState(FALLBACK_EXERCISES);
  ...
  // merge met gelogde namen zodat custom entries ook vindbaar zijn
...
function ExercisePicker({ onClose, onSelect, db }){
  ...
  const filtered=useMemo(()=>{
    const q=norm(query); if(!q) return db.slice(0,100);
    ...
    <div className="fixed inset-0 modal-overlay">
      <div className="hit-card neon-border" style={{maxWidth:720, maxHeight:"90vh", overflowY:"auto"}}>
        ...
        <div style={{position:"relative"}}>
          ...
          <input autoFocus value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Zoek…" className="input" style={{paddingLeft:36, marginTop:8, marginBottom:8}}/>
        </div>
