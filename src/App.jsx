...
function WorkoutsScreen({ onPickDay, data, onDelete, unit }) {
  return (
    <div className="space-y-4">
      ...
      <div>
        <h2 className="text-lg font-semibold mb-2 text-white">Mijn Workouts</h2>
  ...
  return (
    <div className="space-y-3 text-white">
      <div className="border border-neutral-800 rounded-2xl p-4 bg-neutral-950">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setUnit("kg")} className={`px-3 py-2 rounded-xl border ${data.settings?.unit === "kg" ? "bg-red-700 border-red-700" : "border-neutral-800 hover:bg-neutral-900"}`}>KG</button>
          <button onClick={() => setUnit("lbs")} className={`px-3 py-2 rounded-xl border ${data.settings?.unit === "lbs" ? "bg-red-700 border-red-700" : "border-neutral-800 hover:bg-neutral-900"}`}>LBS</button>
        </div>
      </div>
      ...
      <div className="border border-neutral-800 rounded-2xl p-4 bg-neutral-950">
        ...
        <div className="flex items-center gap-2">
          <button onClick={exportData} className="px-3 py-2 rounded-xl border border-neutral-800 hover:border-red-700"><Download className="w-4 h-4 inline mr-2" /> Export</button>
      ...
      <div className="border border-neutral-800 rounded-2xl p-4 bg-neutral-950">
        ...
        <button onClick={clearAll} className="px-3 py-2 rounded-xl border border-red-800 text-red-400 hover:bg-neutral-900">Alles wissen</button>
      </div>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-neutral-950 text-white">
      ...
      {showDayPicker && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center">
          <div className="bg-neutral-950 p-6 rounded-xl border border-neutral-800 w-full max-w-md">
            <h2 className="mb-4 text-white font-semibold">Kies je dag</h2>
            <div className="grid grid-cols-1 gap-2">
              {dayOptions.map((d) => (
                <button key={d.key} onClick={() => pickDay(d.key)} className="text-left p-3 rounded-xl border border-neutral-800 hover:border-red-700 hover:bg-neutral-900 text-white">
                  {d.label}
                </button>
              ))}
            </div>
            <div className="mt-4 text-right">
              <button onClick={() => setShowDayPicker(false)} className="px-4 py-2 rounded-xl border border-neutral-800 hover:bg-neutral-900 text-white">Annuleren</button>
            </div>
          </div>
        </div>
      )}
