...
/* Topbar – logo = home, titel groot/wit */
...
/* Start – KNOPPEN MIDDEN & ONDER ELKAAR, QUOTE ERONDER */
        ...
        {suggestions.length>0 && (
          <div className="text-sm text-muted" style={{marginBottom:8}}>
            Suggesties: {suggestions.map((s,i)=>(
              <button key={s} onClick={()=>onSelect(s)} className="underline" style={{marginRight:8}}>{s}{i<suggestions.length-1?",":""}</button>
            ))}
          </div>
        )}
      ...
      {data.workouts.slice().sort((a,b)=>new Date(b.date)-new Date(a.date)).map(w=>(
        <div key={w.id} className="hit-card">
          ...
          <div style={{marginTop:10}}>
            <button onClick={()=>onDelete(w.id)} className="btn" style={{maxWidth:220}}>Verwijderen <Trash2 style={{width:16, height:16, marginLeft:6}}/></button>
          </div>
        </div>
      ))}
    </div>
...
/* WorkoutForm – gecentreerd */
function WorkoutForm({ dayKey, onSave, onCancel, unit, customExerciseName }){
  const plan = dayKey==="custom" ? {name:`Losse oefening – ${customExerciseName}`, exercises:[{name:customExerciseName, note:"Log je werkset(ten)"}]} : EXERCISE_PLAN[dayKey];
    ...
    <div className="center-col">
      <div className="hit-card">
        ...
        <div style={{display:"flex", gap:8, justifyContent:"center", marginTop:8, flexWrap:"wrap"}}>
          <input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Zoek een oefening…" className="input"/>
          <button onClick={()=>{
            if(query){
